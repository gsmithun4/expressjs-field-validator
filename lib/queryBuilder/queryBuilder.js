...
const validateOps = () => {
  ...
  const setGlobalRemoveIfEmpty = (validationObject) => {
    validationObject.removeIfEmpty = true;
  };
  const setCleanUp = (validationObject) => {
    validationObject.cleanUp = true;
  };
  ...
  const setMatching = (validationObject, regex) => {
    validationObject.isMatching = regex;
  };
...
const responseConfigs = (location) => {
  ...
  const removeIfEmpty = () => {operations.setGlobalRemoveIfEmpty(responseObject); return responseConfigsObj;};
  const cleanUp = () => {operations.setCleanUp(responseObject); return responseConfigsObj;};
...
const param = (paramName) => {
  ...
  const isMatching = (regex) => {operations.setMatching(validationObject, regex); return fieldFunctions;};
