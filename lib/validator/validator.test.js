...
describe('Test for body params', () => {
  ...
  describe('isMatching (RegEx validation)', () => {
    test('Success case - value matches RegExp object', () => {
      const req = {
        body: { code: 'ABC-123' }
      };
      const validation = [
        { param: 'code', isRequired: true, isMatching: /^[A-Z]+-\d+$/ }
      ];
      const response = { mode: 'reject' };
      const validatorfn = validaor('body', validation, response);
      validatorfn(req, resp, next);
    ...
    test('Error case - value does not match RegExp object', () => {
      const req = {
        body: { code: 'abc-123' }
      };
      const validation = [
    ...
    test('Success case - value matches regex string', () => {
      const req = {
        body: { postalCode: '12345' }
      };
      const validation = [
        { param: 'postalCode', isRequired: true, isMatching: '^\\d{5}$' }
      ];
      const response = { mode: 'reject' };
      const validatorfn = validaor('body', validation, response);
      validatorfn(req, resp, next);
    ...
    test('Error case - value does not match regex string', () => {
      const req = {
        body: { postalCode: 'ABCDE' }
      };
      const validation = [
        { param: 'postalCode', isRequired: true, isMatching: '^\\d{5}$' }
      ];
    ...
    test('Success case - numeric value coerced to string and matched', () => {
      const req = {
        body: { score: 42 }
      };
      const validation = [
        { param: 'score', isMatching: /^\d+$/ }
      ];
      const response = { mode: 'reject' };
      const validatorfn = validaor('body', validation, response);
      validatorfn(req, resp, next);
    ...
    test('isMatching is skipped when field is empty and not required', () => {
      const req = {
        body: {}
      };
      const validation = [
        { param: 'code', isMatching: /^[A-Z]+$/ }
      ];
      const response = { mode: 'reject' };
      const validatorfn = validaor('body', validation, response);
      validatorfn(req, resp, next);
    ...
    test('Success case - isMatching on nested object field', () => {
      const req = {
        body: { user: { username: 'john_doe' } }
      };
      const validation = [
        { param: 'user', isObject: true, children: [
          { param: 'username', isRequired: true, isMatching: /^[a-z_]+$/ }
        ]}
      ];
      const response = { mode: 'reject' };
      const validatorfn = validaor('body', validation, response);
    ...
    test('Error case - isMatching on nested object field fails', () => {
      const req = {
        body: { user: { username: 'John Doe!' } }
      };
      const validation = [
        { param: 'user', isObject: true, children: [
          { param: 'username', isRequired: true, isMatching: /^[a-z_]+$/ }
